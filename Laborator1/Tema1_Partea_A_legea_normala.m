...
mu = 0;
sigma = 1; % mu si sigma definesc transformarea liniara g(x)=a*x+b pornind de la legea normala standard
C = 50; % numarul de clase ale histogramei

x = randn(1,N); % tipul de lege de probabilitate de studiat
x = sigma*x + mu; %nu se intampla nimic, in acest caz
...
frecv2 = 1/(sigma*sqrt(2*pi)).*exp(-(x2-mu).^2/2/sigma^2); % am putea zice si normpdf(x2) / distributia normala de medie mu si abatere standard sigma
...
annotation('textarrow',x_arrow,y_arrow,'String',['Max pdf in mu = ',num2str(1/sigma/sqrt(2*pi))]) %aceasta valoare e valabila pentru gaussiana standard; pentru alte gaussiene poate fi omisa
hold off
